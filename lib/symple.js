...
Symple.prototype.authorize = function(socket, req, fn) {
  ...
  if (self.config.authentication) {
    ...
    self.getSession(req.token, function(err, session) {
      if (err) {
        debug(socket.id, 'authentication failed: redis error');
        return fn(401, 'Authentication failed');
      }
      if (typeof session !== 'object') {
        debug(socket.id, 'authentication failed: no session');
        return fn(401, 'Authentication failed');
      }
      else {
...
function get_users_by_room(io, nsp, room) {
  var users = []
...
function get_ids_by_room(io, nsp, room) {
  var users = []
  for (var id in io.of(nsp).adapter.rooms[room]) {
    users.push(id);
  };
...
Symple.prototype.broadcast = function(socket, message) {
  ...
  if (to === undefined) {
    ...
    debug(socket.id, 'sending direct message:', to);
    // debug(socket.id, 'sending direct message ROOM:' );
    //
    //     debug(socket.id, 'broadcasting rooms!!!!!!!!!!!!!!!!!!:', get_ids_by_room(this.io, '/', socket.id));
    //
    //         debug(socket.id, 'broadcasting @@@@@@@rooms!!!!!!!!!!!!!!!!!!:', get_ids_by_room(this.io, '/', to));
    // // socket.rooms


    // this.io.in(to).clients(function(error, clients){
    ...
    // socket.broadcast.json.to(to).send(message);
    // this.io.to(to).emit('message', message);
    // socket.broadcast.to(to).json.send(message); //.except(this.unauthorizedIDs())
  }
  ...
  // if (typeof to !== 'object' || to.group === undefined) {
  //   debug(socket.id, 'dropping invalid message without destination:', to, ':', message);
  //   return;
  // }
