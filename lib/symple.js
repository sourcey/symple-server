...
Symple.prototype.initRedis = function() {
  ...
  if (this.config.redis) {
    ...
    // this.io.adapter(adapter({ key: 'symple', this.pub: this.pub, this.sub: this.sub }));
...
Symple.prototype.onDisconnect = function(socket) {
  ...
  if (socket.peer) {
    ...
    if (socket.peer.online) {
      ...
      const p = socket.peer.toPresence();
...
Symple.prototype.getSession = function(token, fn) {
  this.redis.get('symple:session:' + token).then((value) => {
    // console.log('getSession redisss', 'symple:session:' + token, value)
    if (value === null) {
      fn('No session', null);
    } else {
