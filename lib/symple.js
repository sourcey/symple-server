...
Symple.prototype.initRedis = function() {
  ...
  if (this.config.redis && !this.redis) {
    ...
    debug('init redis:', this.config.redis.port, this.config.redis.host);
    this.redis = redis(this.config.redis.port, this.config.redis.host, opts);
...
Symple.prototype.broadcast = function(socket, message) {
  ...
  if (typeof to === 'undefined') {
    ...
    socket.to(addr.user || addr.id).emit('message', message); //.except(this.unauthorizedIDs())
  }
  else if (Array.isArray(to)) {
    // send to an multiple rooms
    for (var room in to) {
      socket.to(to[room]).emit('message', message);
    };
  }
  else {
...
Symple.prototype.getSession = function(token, fn) {
  ...
  // console.log('getSession redisss', this.redis)
  // , function(err, reply) {
  //   if (reply) {
