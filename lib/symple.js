...
Symple.prototype.authorize = function(socket, req, fn) {
  ...
  if (self.config.authentication) {
    if (!req.user || !req.token) {
      debug(socket.id, 'authentication failed: no user or token');
  ...
  // Anonymous access
    ...
    if (!req.user) {
      debug(socket.id, 'authentication failed: no user');
      return fn(400, 'Bad request');
    }
...
Symple.prototype.broadcast = function(socket, message) {
  ...
  if (to === undefined) {
    ...
    socket.broadcast.json.to(to).emit('message', message); //.except(this.unauthorizedIDs())
  }
